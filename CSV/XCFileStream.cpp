...
int main() 
{
	...
	//读文件  
	//ifstream inFile("D:\\study\\project\\LiDAR_CSV\\2021-7-13-20-23-47R-csv\\Frame2.csv", ios::in);//inFile来自fstream,ifstream为输入文件流(从文件读入)
...
/*	for (int i = 0; i < files.size(); i++)
	...
	//		cout<<lineStr<<endl;
		...
		//	cout << typeid(lineArray[5].c_str()).name()<<endl;
			if (a > 2)
			{
				if ((35000<stol(lineArray[5]))&&(stol(lineArray[5])<35100)&&(lineArray[4]=="0"))
				{
					cout << lineArray[5].c_str()<<endl;
...
/*	for(int j = 0; j < strArray[1].size(); j++)
	{ 
		cout << strArray[1][j]<<" ";
		cout << endl;
	}*/
...
//	ofstream out0(pathorigin.append("0").append(to_string(P1.azimuth)).append(".csv"));
//	ofstream file1("D:\\study\\project\\LiDAR_CSV\\result\\ZONGc.csv",ios::app);
